...
void * Allocation::getPtr(const Context *rsc, uint32_t lod, RsAllocationCubemapFace face,
...
void rsi_AllocationGetPtr(Context *rsc, RsAllocation valloc,
                          uint32_t lod, RsAllocationCubemapFace face,
                          uint32_t z, uint32_t array, size_t *stride, size_t strideLen) {
    Allocation *alloc = static_cast<Allocation *>(valloc);
    assert(strideLen == sizeof(size_t));

    alloc->getPtr(rsc, lod, face, z, array, stride);
}
