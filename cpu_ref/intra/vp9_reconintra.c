...
static void build_intra_predictors_rs(const INTRA_PARAM*param, const uint8_t *ref,
  ...
  if (plane == 0) {
    frame_width = param->y_width;
    frame_height = param->y_height;
  } else {
    frame_width = param->uv_width;
    frame_height = param->uv_height;
  }

  // Get block position in current frame.
  x0 = (-param->mb_to_left_edge >> (3 + param->subsampling_x)) + x;
  y0 = (-param->mb_to_top_edge >> (3 + param->subsampling_y)) + y;
    ...
    if (param->mb_to_right_edge < 0) {
      ...
      if (x0 + 2 * bs <= frame_width) {
        if (right_available && bs == 4) {
          memcpy(above_row - 1, above_ref - 1, 2 * bs + 1);
        } else {
          memcpy(above_row - 1, above_ref - 1, bs + 1);
          memset(above_row + bs, above_row[bs - 1], bs);
        }
      } else if (x0 + bs <= frame_width) {
        const int r = frame_width - x0;
        if (right_available && bs == 4) {
          memcpy(above_row - 1, above_ref - 1, r + 1);
          memset(above_row + r, above_row[r - 1],
                     x0 + 2 * bs - frame_width);
        } else {
          memcpy(above_row - 1, above_ref - 1, bs + 1);
          memset(above_row + bs, above_row[bs - 1], bs);
...
void vp9_predict_intra_block_rs(const INTRA_PARAM *param, int block_idx, int bwl_in,
  ...
  const int have_top = (block_idx >> bwl) || param->up_available;
  const int have_left = (block_idx & wmask) || param->left_available;
  const int have_right = ((block_idx & wmask) != wmask);
  const int x = aoff * 4;
  const int y = loff * 4;

  build_intra_predictors_rs(param, ref, ref_stride, dst, dst_stride, (MB_PREDICTION_MODE)mode, tx_size,
